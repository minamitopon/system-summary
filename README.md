# system-summary

## システムビューア

`main` ブランチのシステムノートは、GitHub Pages のスマートフォン対応ビューアから閲覧できます。

各項目は独立して展開でき、開いた状態は使用中の端末に保存されます。検索はすべての `.bml` を対象にします。

ビューアは `.bml` を直接読み込むため、これまで通りシステムの原文を変更して `main` にマージすれば、サイトにも自動で反映されます。

## ブランチルール
### main
リリース後のシステムのみが存在する。  
mainブランチにあるサマリーを最新のものとし、その他のブランチで議論中のシステム等は使用しない。  

### development
開発ブランチは基本的にここから切る。  
developmentにマージされたもののみ、mainに組み込むことが可能。  

#### feature/xxxxxx
システムの改修用ブランチ。  
developmentブランチから新規に作成し、該当箇所を編集後プルリクを作成する。  
レビュアーは、自分以外の参加者。  
提案に関する議論は、そのプルリク内で行う。  
全員から承認を得られたら、レビュイーがdevelopmentブランチにマージする。  

#### bugfix/xxxxxx
システムの誤植等を直すブランチ。  
developmentブランチから新規に作成し、該当箇所を編集後プルリクを作成する。  
レビュアーは、Shellingまたはpon。  
あくまでサマリーの体裁を整えるための修正を行うブランチなので、システムの改修を伴う場合はfeatureブランチで作業する。  
